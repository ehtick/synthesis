...
extern "C" {
	...
	EXPORT bool Physics_Instance_Exists() {
		return synthesis::Manager != nullptr;
	}
	
	EXPORT void Physics_Create_Instance(bool forceNew) {
		synthesis::PhysicsManager::CreateInstance(forceNew);
	}

	EXPORT void Physics_Delete_Instance() {
		synthesis::PhysicsManager::DeleteInstance();
	}

	EXPORT void Physics_Step(float deltaT) {
		// synthesis::Manager->Step(deltaT);
		synthesis::Manager->Step(deltaT);
	}

	EXPORT void Physics_Destroy_Simulation() {
		synthesis::Manager->DestroySimulation();
	}
	...
	EXPORT RayHitClosest Physics_Ray_Cast_Closest(Vec3 from, Vec3 to) {
		...
		synthesis::Manager->dynamicsWorld->rayTest(callback.m_rayFromWorld, callback.m_rayToWorld, callback);
		// btCollisionWorld::ClosestRayResultCallback callback(To_Bullet_Vector_3(from), To_Bullet_Vector_3(to));
		// synthesis::Manager->rayCastClosest(callback.m_rayFromWorld, callback.m_rayToWorld, callback);
		return {
			callback.hasHit(),
	...
	EXPORT Vec3 Get_World_Gravity() {
		return To_Vector_3_Native(synthesis::Manager->getGravity());
	}

	EXPORT void Set_World_Gravity(Vec3 gravity) {
		synthesis::Manager->setGravity(To_Bullet_Vector_3(gravity));
	}

	EXPORT int Get_Physics_Num_Iterations() {
		return synthesis::Manager->GetNumIteration();
	}
	EXPORT void Set_Physics_Num_Iterations(int numIter) {
		synthesis::Manager->SetNumIteration(numIter);
	}
	...
	EXPORT void Delete_Compound_Collision_Shape(void* compoundShape) {
		std::vector<btCollisionShape*> shapes;
		btCompoundShape* s = (btCompoundShape*)compoundShape;
		for (int i = 0; i < s->getNumChildShapes(); i++) {
			shapes.push_back(s->getChildShape(i));
		}
		delete s;
	...
	EXPORT void Delete_RigidBody(void* rigidbody) {
		synthesis::Manager->DestroyRigidbody((btRigidBody*)rigidbody);
	}
	...
	EXPORT void Delete_Hinge_Constraint(void* hinge) {
		...
		if (constraint != nullptr) {
			synthesis::Manager->DestroyConstraint(constraint);
		}
	}
	...
	EXPORT void* Create_RigidBody_Dynamic(float mass, void* collisionShape) {
		return synthesis::Manager->createDynamicRigidBody((btCollisionShape*)collisionShape, btScalar(mass));
	}
	EXPORT void* Create_RigidBody_Static(void* collisionShape) {
		return synthesis::Manager->createStaticRigidBody((btCollisionShape*)collisionShape);
	}
	...
	{
		return synthesis::Manager->createHingeConstraint((btRigidBody*)bodyA, (btRigidBody*)bodyB,
